...
static void write_cmd_cb(struct bt_conn *conn, void *user_data)
{
	...
	if (delta > (1U * NSEC_PER_SEC)) {
		printf("%s: count= %u, len= %u, rate= %u bps.\n", __func__,
...
static int mtu_exchange(struct bt_conn *conn)
{
	...
	printf("%s: Current MTU = %u\n", __func__, bt_gatt_get_mtu(conn));

	mtu_exchange_params.func = mtu_exchange_cb;

	printf("%s: Exchange MTU...\n", __func__);
	err = bt_gatt_exchange_mtu(conn, &mtu_exchange_params);
	if (err) {
		printf("%s: MTU exchange failed (err %d)", __func__, err);
	}
...
{
	printf("%s: int (0x%04x, 0x%04x) lat %u to %u\n", __func__,
...
{
	printf("%s: int 0x%04x lat %u to %u\n", __func__, interval,
	       latency, timeout);
}
